...
class FriendsRepository(IFriendsRepository):
    def insert(self, sender_id, receiver_id):
        if Friends.objects.create(sender_id=sender_id, receiver_id=receiver_id, state=0):
            return True
        return False
    ...
    def accept(self, sender_id: int, receiver_id: int):
        if Friends.objects.filter(sender_id=sender_id, receiver_id=receiver_id).update(state=1):
            return True
        return False
    def reject(self, sender_id: int, receiver_id: int):
        pass
    ...
    def set_state(self, sender_id: int, receiver_id: int, state: int):
        ...
        return False
    def get_double(self, sender_id: int, receiver_id: int):
        fr1 = Friends.objects.filter(sender_id=sender_id, receiver_id=receiver_id).first()
        fr2 = Friends.objects.filter(sender_id=receiver_id, receiver_id=sender_id).first()
        if fr1:
            return fr1
        if fr2:
            return fr2
