...
class IFriendsService(ABC):
    @abstractmethod
    def add_friend(self, sender_id, receiver_id) -> tuple[dict[str, str], bool]:
        pass

    @abstractmethod
    def delete_friend(self, sender_id, receiver_id) -> tuple[dict[str, str], bool]:
        pass

    @abstractmethod
    def accept_request(self, sender_id, receiver_id) -> tuple[dict[str, str], bool]:
        pass

    @abstractmethod
    def reject_request(self, sender_id, receiver_id) -> tuple[dict[str, str], bool]:
        pass
    ...
    @abstractmethod
    def get_friends(self, user_id, page, limit) -> tuple[dict[str, str], bool]:
        pass
    ...
    def get_requests(self, user_id, page, limit) -> tuple[dict[str, str], bool]:
        ...
        try:
            ...
            if e is Exception.EmptyPage:
                return BaseResponse(True, "No friend requests found in this page", None).res()
            return BaseResponse(True, "Failed to get friend requests", None).res()
        if not requests:
            return BaseResponse(True, "No friend requests found", None).res()
        res = FriendsSerializer.response(requests)
        paginate_data = {
            "current_page": page,
            "page_size": limit,
