            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (errorData.error === 'Token has expired') {
                        ...
                        await RefreshToken();
                        return fetchUsers(); // Retry fetching after token refresh
                    } else {
                ...
                if (error.message === 'Token has expired') {
                    console.log('Token expired, refreshing...');
                    await RefreshToken();
                    fetchUsers(); // Retry fetching after token refresh
                } else {
                    alert(error.message);
                }
            }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (errorData.error === 'Token has expired') {
                console.log('Token expired, refreshing...');
                await RefreshToken();
                return addUserAsFriend(userId); // Retry adding after token refresh
            } else {
                throw new Error(errorData.error);
            }
        }
